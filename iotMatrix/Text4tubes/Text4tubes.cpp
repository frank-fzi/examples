/*
	Text4tubes.cpp - Library for adding text to PixelTubes
	fork of LEDText V6 class by Aaron Liddiment (c) 2015
	modified by M. Frank, 2018
*/
...
Text4tubes::Text4tubes(PixelTubes *tubes) {
	_tubes = tubes;
}

void Text4tubes::SetFont(const uint8_t *FontData)
{
...
void Text4tubes::Init(uint8_t width, uint8_t height, uint8_t originX, uint8_t originY)
{
  m_XMin = originX;
  m_YMin = originY;
...
void Text4tubes::SetBackgroundMode(uint16_t Options, uint8_t Dimming)
{
...
void Text4tubes::SetScrollDirection(uint16_t Options)
{
  m_Options = (m_Options & (~SCROLL_MASK)) | (Options & SCROLL_MASK);
}


void Text4tubes::SetTextDirection(uint16_t Options)
{
  m_Options = (m_Options & (~CHAR_MASK)) | (Options & CHAR_MASK);
}


void Text4tubes::SetTextColrOptions(uint16_t Options, uint8_t ColA1, uint8_t ColA2, uint8_t ColA3, uint8_t ColB1, uint8_t ColB2, uint8_t ColB3)
{
...
void Text4tubes::SetFrameRate(uint8_t Rate)
{
  m_FrameRate = Rate;
}


void Text4tubes::SetOptionsChangeMode(uint16_t Options)
{
  m_Options = (m_Options & (~INSTANT_OPTIONS_MODE)) | (Options & INSTANT_OPTIONS_MODE);
}


void Text4tubes::SetText(unsigned char *Txt, uint16_t TxtSize)
{
  m_pText = Txt;
  m_pSize = TxtSize;
...
void Text4tubes::DecodeOptions(uint16_t *tp, uint16_t *opt, uint8_t *backDim, uint8_t *col1, uint8_t *col2, uint8_t *colDim, uint8_t *RC)
{
  switch (m_pText[*tp])
  {
...
int Text4tubes::UpdateText(DisplayMode displayMode) {
            ...
            if ((y >= 0) && (y < _tubes->getSize())) {
                ...
                if ((opt & COLR_MASK) != COLR_EMPTY) {
                  if ((opt & COLR_MASK) == COLR_DIMMING) {
                    ...
                    if ((opt & COLR_HSV) == COLR_RGB) {
						_tubes->setPixel(x,y,CRGB(v[0], v[1], v[2]),displayMode);
						//(*m_Matrix)(x, y) = CRGB(v[0], v[1], v[2]);
                    } else {
						_tubes->setPixel(x,y,CHSV(v[0], v[1], v[2]),displayMode);
              ...
              // Fix for double dimming/blanking of blank vertical gap lines
                if ((opt & BACKGND_MASK) == BACKGND_ERASE) {
					_tubes->setPixel(x,y, CRGB(0, 0, 0),displayMode);
